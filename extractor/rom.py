...
##                        [-1, 'unknown', ByteData, 1], # There is 1 more byte, padding?
    ...
    '''
    def _read_text(self, offset, length):
        self.seek(offset)
    ...
    def _read_rom_address_list_wolf(self, offset, count):
        self.seek(offset)
        offsets = []
        for x in range(count):
            ...
            assert 1 <= zb <= 3
            zb -= 1
            buf = '\00' * zb + self.read(4 - zb)
            offsets.append(struct.unpack('<I', buf)[0] - 0xc00000)
        return offsets
    ...
    '''
    def _store_and_get_length(self, store, value):
        del store[:]
        store.extend(value)
        return len(store)
